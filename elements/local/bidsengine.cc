...
#include <arpa/inet.h>
#include <sys/time.h>
#include "bidsengine.hh"
CLICK_DECLS

BIDSEngine::BIDSEngine() : _test(0),
			     _batcher(0)
{
    _anno_offset = -1;
    _slice_offset = -1;
...
BIDSEngine::~BIDSEngine()
{
}

int
BIDSEngine::configure(Vector<String> &conf, ErrorHandler *errh)
{
    _div = 0;
...
char**
BIDSEngine::generate_random_patterns(int np, int plen)
{
    struct timeval tv;
    gettimeofday(&tv, 0);
...
int
BIDSEngine::initialize(ErrorHandler *errh)
{
    if (_on_cpu == 3)
	return 0;
	...
	if (_anno_offset < 0) {
	    ...
	    errh->message("BIDSEngine anno offset %d", _anno_offset);
	...
	if (_slice_offset < 0) {
	    ...
	    errh->message("BIDSEngine slice offset %d", _slice_offset);
    } else {
	_anno_offset = 0;
...
void
BIDSEngine::bpush(int i, PBatch *p)
{
    if (_on_cpu <= 0) {
	if (!p->dev_stream)
...
void
BIDSEngine::push(int i, Packet *p)
{
    if (_on_cpu != 3) {
